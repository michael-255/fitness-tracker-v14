...
<template>
  <QTable :rows="tableRows" :columns="tableColumns" :rows-per-page-options="[10]">
    <!-- Table Header -->
    <template v-slot:top>
      <div class="q-table__title text-weight-bold">Workouts</div>
      <QSpace />
      <div>
        <QBtn color="negative" label="Clear Workouts" @click="clearTableData()" />
      </div>
    </template>
    <!-- Column Headers -->
  ...
  >
    <QCard class="bg-primary text-white">
      ...
      <QCardSection>
        <div class="text-h6">Workout Details</div>
      </QCardSection>
